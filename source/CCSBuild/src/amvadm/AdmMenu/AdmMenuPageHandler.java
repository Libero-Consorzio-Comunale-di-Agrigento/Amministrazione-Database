 ...
 * -------------------------- */
String progettoPrecedente = "";
String moduloPrecedente = "";

if (SessionStorage.getInstance(e.getPage().getRequest()).getAttribute("MVSP") != null) {
	progettoPrecedente = SessionStorage.getInstance(e.getPage().getRequest()).getAttribute("MVSP").toString();
}
if (SessionStorage.getInstance(e.getPage().getRequest()).getAttribute("MVSM") != null) {
	moduloPrecedente = SessionStorage.getInstance(e.getPage().getRequest()).getAttribute("MVSM").toString();
}
...
if (e.getPage().getHttpPostParameter("SM") != null) {
	...
	// Se il progetto selezionato è diverso da quello precedente il modulo selezionato di default va controllato
	if (!SessionStorage.getInstance(e.getPage().getRequest()).getAttribute("MVSP").toString().equals(progettoPrecedente)) {
