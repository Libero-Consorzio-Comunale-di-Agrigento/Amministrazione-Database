...
//load @30-3CB745FF
    boolean load() {
        ...
        command.setCountSql( "SELECT COUNT(*) FROM ( SELECT m.descrizione||' - '||i.descrizione servizio , u.nominativo richiedente ,  "
                                                         ...
                                                         + "            r.indirizzo_notifica indirizzo_notifica , decode(r.notificata,'S','',  "
                                                         + "            decode(instr(u.note,'PWD='),1,substr(u.note, 5,  "
                                                         + "            length(u.note)) ,decode(r.stato,'A','PWDUTENTE='||u.password,'')) ) seconda_pwd,  "
                                                         + "            r.note note FROM ad4_richieste_abilitazione r, ad4_istanze i, ad4_moduli m,  "
...
//loadDataBind @30-CAA8828A
        if ( records == null || ! records.hasMoreElements() ) {
            ...
            while ( counter < rows.length && records.hasMoreElements() ) {
                ...
                row.setSECONDA_PWD(Utils.convertToString(ds.parse(record.get("SECONDA_PWD"), row.getSECONDA_PWDField())));
