...
//AD4_RICHIESTE_ABILITAZIONEGrid Grid: method bind @30-B61FBD5B
        public void bind(com.codecharge.components.Component model, AD4_RICHIESTE_ABILITAZIONEGridRow[] rows ) {
            ...
            while ( counter < rows.length && rows[counter] != null ) {
                ...
                c = (com.codecharge.components.Control) hashRow.get("SECONDA_PWD");
                if ( c == null ) { 
                    c = (com.codecharge.components.Control) model.getControl("SECONDA_PWD").clone();
                    c.setValue(row.getSECONDA_PWD());
                    hashRow.put( c.getName(), c );
                }
...
//AD4_RICHIESTE_ABILITAZIONEGrid Directory: getRowFieldByName @30-426DAB02
        public Object getRowFieldByName( String name, AD4_RICHIESTE_ABILITAZIONEGridRow row ) {
            ...
            } else if ( "SECONDA_PWD".equals(name) ) {
                value = row.getSECONDA_PWD();
...
//AD4_RICHIESTE_ABILITAZIONERecord Record: children actions @7-DDC9D848
            ...
            if ("AD4_RICHIESTE_ABILITAZIONERecord".equals(formName)) {
                ...
                if (mode != -1) { // Update mode
                    ...
                    if (page.getParameter("Archivia") != null) {
                        if (validate()) {
                            ArchiviaAction();
                            if ( !model.hasErrors()) return page.getRedirectString();
                        }
...
//Archivia Button @75-FEA3DCDC
        void ArchiviaAction() {
            com.codecharge.components.Button buttonModel = (com.codecharge.components.Button) model.getChild("Archivia");
            Vector excludeParams = new Vector(buttonModel.getExcludeParams());
            excludeParams.addAll(model.getExcludeParams());
            page.setRedirectString( page.getActionPageName() + Names.ACTION_SUFFIX + "?" + page.getHttpGetParams().toString( excludeParams ) );
            buttonModel.fireOnClickEvent();
...
//AD4_RICHIESTE_ABILITAZIONERecord Record: init DataSource @7-3558079F
            if (!model.isAllowRead()) return;
            boolean isErrors = false;
...
//AD4_RICHIESTE_ABILITAZIONERecord Record: bind @7-7EE8D2F8
...
//AD4_RICHIESTE_ABILITAZIONERecord Record: getRowFieldByName @7-C3C23E7A
            public Object getRowFieldByName( String name, AD4_RICHIESTE_ABILITAZIONERecordRow row ) {
                Object value = null;
                if ( "USER_ORACLE".equals(name) ) {
                    value = row.getUSER_ORACLE();
                } else if ( "REVISIONE_PARAMETRI".equals(name) ) {
...
//AD4_RICHIESTE_ABILITAZIONERecord Record: method update body @7-4AF14B4F
            if (!model.isAllowUpdate()) return;
            boolean isErrors = false;
            AD4_RICHIESTE_ABILITAZIONERecordDataObject ds = new AD4_RICHIESTE_ABILITAZIONERecordDataObject(page);
            ds.setComponent( model );
...
//AD4_RICHIESTE_ABILITAZIONERecord Record: method validate @7-42746B5C
        boolean validate() {
            model.fireOnValidateEvent();
            boolean isControlError = false;

            com.codecharge.components.Hidden USER_ORACLE = (com.codecharge.components.Hidden) model.getChild( "USER_ORACLE" );
