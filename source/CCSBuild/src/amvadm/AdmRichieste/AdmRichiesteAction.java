...
//AdmRichiesteAction: call children actions @1-57FBD769
        ...
        if (result == null) {
            ...
            TAB_FOLDER.perform(page.getGrid("TAB_FOLDER"));
        }
        if (result == null) {
        ...
        if ( page.getChild( "AdmRichiesteStampa" ).isVisible() ) {
            page.getRequest().setAttribute("AdmRichiesteStampaParent",page);
            amvadm.AdmRichiesteStampa.AdmRichiesteStampaAction AdmRichiesteStampa = new amvadm.AdmRichiesteStampa.AdmRichiesteStampaAction();
            result = result != null ? result : AdmRichiesteStampa.perform( req, resp,  context );
            page.setCookies();
        }
...
//TITLEGrid Grid: set where parameters @148-14693CBC
            try {
...
//AD4_RICHIESTE_ABILITAZIONE Grid: set where parameters @30-0CA8DA18
            try {
                ds.setUrlMVAV( page.getHttpGetParams().getParameter("MVAV"), page.getCCSLocale().getFormat("Integer") );
            } catch ( java.text.ParseException pe ) {
                model.addError( "Invalid parameter 'MVAV'" );
...
//TAB_FOLDER Grid @186-C5A9F5EB
    final class TAB_FOLDERClass {
        com.codecharge.components.Grid model;
        Event e;
//End TAB_FOLDER Grid

//TAB_FOLDER Grid: method perform @186-B48879D3
        protected String perform(com.codecharge.components.Grid model) {
            if ( ! model.isVisible() ) { return null; }
            this.model = model;
            //e = new ActionEvent( model, page );
...
//End TAB_FOLDER Grid: method perform

//TAB_FOLDER Grid: method read: head @186-A74AB037
        boolean read() {
            boolean isErrors = false;
            if ( ! model.allowRead ) return true;
            model.fireBeforeSelectEvent();
//End TAB_FOLDER Grid: method read: head

//TAB_FOLDER Grid: method read: init @186-EB5C1F51
            if ( ! model.allowRead ) return true;
            TAB_FOLDERDataObject ds = new TAB_FOLDERDataObject(page);
            ds.setComponent( model );
//End TAB_FOLDER Grid: method read: init

//TAB_FOLDER Grid: set where parameters @186-6CBA333D
            ds.setUrlNOTIFICATA( page.getHttpGetParams().getParameter("NOTIFICATA") );
            ds.setUrlTIPO_NOTIFICA( page.getHttpGetParams().getParameter("TIPO_NOTIFICA") );
            ds.setUrlMVAV( page.getHttpGetParams().getParameter("MVAV") );
            ds.setUrlIST( page.getHttpGetParams().getParameter("IST") );
            ds.setUrlMOD( page.getHttpGetParams().getParameter("MOD") );
            ds.setUrlSTATO( page.getHttpGetParams().getParameter("STATO") );
//End TAB_FOLDER Grid: set where parameters

//TAB_FOLDER Grid: set grid properties @186-11C2D650
            
            ds.setPageNum( (model.getPage() < 1 ? 1 : model.getPage()) );
            ds.setPageSize( model.getFetchSize() );
//End TAB_FOLDER Grid: set grid properties

//TAB_FOLDER Grid: retrieve data @186-FE4E4327
            ds.load();
            if ( ! ds.isEmpty() ) {
                bind( model, ds.getRows());
            }
            model.setAmountOfRows( ds.getAmountOfRows() );
//End TAB_FOLDER Grid: retrieve data

//TAB_FOLDER Grid: check errors @186-B2B22DDA
            isErrors = ds.hasErrors();
            if ( isErrors ) model.addErrors( ds.getErrors() );
//End TAB_FOLDER Grid: check errors

//TAB_FOLDER Grid: method read: tail @186-F575E732
            return ( ! isErrors );
        }
//End TAB_FOLDER Grid: method read: tail

//TAB_FOLDER Grid: method bind @186-D03B101A
        public void bind(com.codecharge.components.Component model, TAB_FOLDERRow[] rows ) {
            if ( model == null || rows == null ) return;
            ArrayList listRows = (ArrayList) ((ArrayList) model.getChildRows()).clone();
            Iterator childRows = listRows.iterator();
            int counter = 0;
            TAB_FOLDERRow row = null;
            while ( counter < rows.length && rows[counter] != null ) {
                row = rows[counter++];
                HashMap hashRow = null;
                com.codecharge.components.Control c = null;
                ...
                c = (com.codecharge.components.Control) hashRow.get("TAB");
                if ( c == null ) { 
                    c = (com.codecharge.components.Control) model.getControl("TAB").clone();
                    c.setValue(row.getTAB());
                    hashRow.put( c.getName(), c );
                }
...
//End TAB_FOLDER Grid: method bind

//TAB_FOLDER Directory: getRowFieldByName @186-CDBE8E6E
        public Object getRowFieldByName( String name, TAB_FOLDERRow row ) {
            Object value = null;
            if ( "TAB".equals(name) ) {
                value = row.getTAB();
            }
...
//End TAB_FOLDER Directory: getRowFieldByName

//TAB_FOLDER Grid: method validate @186-104025BA
        boolean validate() {
            return true;
        }
//End TAB_FOLDER Grid: method validate
...
//AD4_RICHIESTE_SERVIZIO Grid: set where parameters @123-A22A40B5
            try {
                ds.setSesGroupID( SessionStorage.getInstance(req).getAttribute("GroupID"), page.getCCSLocale().getFormat("Integer") );
            } catch ( java.text.ParseException pe ) {
                model.addError( "Invalid parameter 'GroupID'" );
...
//AD4_RICHIESTE_SERVIZIO Grid: method bind @123-8E7B4B72
...
//AD4_RICHIESTE_SERVIZIO Directory: getRowFieldByName @123-0C65A72E
