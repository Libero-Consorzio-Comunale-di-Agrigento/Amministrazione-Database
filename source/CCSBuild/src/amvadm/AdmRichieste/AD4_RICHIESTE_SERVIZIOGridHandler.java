 ...
 * -------------------------- */
 ...
 if (
      (!(e.getPage().getRequest().getParameter("IST") != null))
      ||(
	     ("2".equals(e.getPage().getRequest().getParameter("MVAV")))
	   &&("N".equals(e.getPage().getRequest().getParameter("NOTIFICATA")))
	   &&("POSTA".equals(e.getPage().getRequest().getParameter("TIPO_NOTIFICA")))
	   )
	 ) {
  e.getGrid().setVisible(false);
