...
//          String name = recode((String) paramNames.nextElement());
    ...
    private void convertPostToArray() {
       ...
       private Hashtable parseUrlEncoded(String urlEncodedParameters) {
        ...
        if(urlEncodedParameters.startsWith("_h=")){

            try{
             urlEncodedParameters = StringUtils.decryptB64(urlEncodedParameters.split("=")[1].replaceAll("%3D","=").replaceAll("%2F","/"));
             urlEncodedParameters =  urlEncodedParameters.replaceAll("\\?","");
             urlEncodedParameters =  urlEncodedParameters.replaceAll(" ","");
             }catch(Exception e){
			 }
        }
