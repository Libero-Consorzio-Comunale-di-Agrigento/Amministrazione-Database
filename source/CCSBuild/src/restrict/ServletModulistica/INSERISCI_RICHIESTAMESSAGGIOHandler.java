...
if (e.getControl().getValue().toString().substring(0,3).equals("ID=")) {
   ...
   SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVIDRIC",id);
	...
	boolean modelliPrecedenti = "true".equals((String) e.getPage().getRequest().getAttribute("MODELLI_PRECEDENTI"));
	...
	// In caso di modelli successivi non mostro il messaggio che conterrebbe il link di stampa
	if (modelliPrecedenti) {
		e.getControl().setValue("caricamento in corso..");
		//System.out.println("Before Show Label:modelliSuccessivi" + modelliSuccessivi);
	}
