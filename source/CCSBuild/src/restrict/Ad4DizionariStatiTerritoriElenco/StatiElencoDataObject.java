//StatiElenco DataSource @8-C35BC860
...
//load @8-8EF5695C
    boolean load() {
        ...
        command.setCountSql( "SELECT COUNT(*) FROM ( select stato_territorio ,denominazione ,sigla ,desc_cittadinanza ,utente_aggiornamento ,ad4_ccs.format_data_trunc(data_aggiornamento) data_aggiornamento ,  "
                                                         + "            '<a class=\"AFCDataLink\"' ||' href=\"' ||'Ad4DizionariComuni.do' ||'?'||'MVPG=Ad4DizionariComuniElenco' ||'&'||'s_PROVINCIA='||stato_territorio ||'\"' ||'>' ||'<img border=0 src=\"../images/freccia_destra.gif\" title=\"vai a comuni\">' ||'</a>' ||'&'||'nbsp;' link_comuni from ad4_stati_territori where ( denominazione like upper('%'||'{s_STATO}')||'%' or stato_territorio like upper('{s_STATO}')||'%' )  ) cnt " );
