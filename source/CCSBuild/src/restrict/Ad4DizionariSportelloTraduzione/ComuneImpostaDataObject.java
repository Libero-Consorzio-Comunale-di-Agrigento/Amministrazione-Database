//ComuneImposta DataSource @5-71A1C77E
...
//attributes of DataObject @5-BD1D1970
    

    TextField urlABI = new TextField(null, null);
    
    TextField urlCAB = new TextField(null, null);
...
//properties of DataObject @5-1CEB3632
    ...
    public void  setUrlCAB( String param ) {
        this.urlCAB.setValue( param );
    }

    public void  setUrlCAB( Object param ) {
        this.urlCAB.setValue( param );
    }

    public void  setUrlCAB( Object param, Format ignore ) {
        this.urlCAB.setValue( param );
    }
...
//load @5-478E00EC
    boolean load() {
        ...
        command.setSql( "select abi "
                    ...
                    + "      ,descrizione "
                    + "     ,  "
                    + "AD4_CCS.GET_TRADUZIONE('SPORTELLI','DESCRIZIONE',ABI||'|'||CAB,'{LINGUA}') DENOMINAZIONE_ALT  "
                    + " from SPORTELLI "
                    + "where ABI= '{ABI}' "
                    + "   and CAB = '{CAB}'" );
        if ( StringUtils.isEmpty( (String) urlCAB.getObjectValue() ) ) urlCAB.setValue( "" );
        command.addParameter( "CAB", urlCAB, null );
...
//insert @5-57C19791
        boolean insert() {
            ...
            command.setSql( "{call AD4_CCS.SPORTELLO_TRADUZIONE_INS(?,?,?,?)}" );
            if ( StringUtils.isEmpty( (String) urlABI.getObjectValue() ) ) urlABI.setValue( "" );
            command.addParameter( "IN_ABI", urlABI, java.sql.Types.CHAR, 0, SPParameter.INPUT_PARAMETER );
            if ( StringUtils.isEmpty( (String) urlCAB.getObjectValue() ) ) urlCAB.setValue( "" );
            command.addParameter( "IN_CAB", urlCAB, java.sql.Types.CHAR, 0, SPParameter.INPUT_PARAMETER );
            if ( StringUtils.isEmpty( (String) urlLINGUA.getObjectValue() ) ) urlLINGUA.setValue( "" );
...
//delete @5-DBFDD7FA
        boolean delete() {
            ...
            command.setSql( "{call AD4_CCS.SPORTELLO_TRADUZIONE_DEL(?,?,?)}" );
            if ( StringUtils.isEmpty( (String) urlABI.getObjectValue() ) ) urlABI.setValue( "" );
            command.addParameter( "IN_ABI", urlABI, java.sql.Types.CHAR, 0, SPParameter.INPUT_PARAMETER );
            if ( StringUtils.isEmpty( (String) urlCAB.getObjectValue() ) ) urlCAB.setValue( "" );
            command.addParameter( "IN_CAB", urlCAB, java.sql.Types.CHAR, 0, SPParameter.INPUT_PARAMETER );
...
//getParameterByName @5-9613E579
        public Parameter getParameterByName(String name, ParameterSource parameterSource) {
            ...
            if ( "CAB".equals(name) && "url".equals(prefix) ) {
                param = urlCAB;
