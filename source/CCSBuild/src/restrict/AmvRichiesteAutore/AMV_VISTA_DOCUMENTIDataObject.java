...
//load @5-A7F4F84B
    boolean load() {
        ...
        command.setCountSql( "SELECT COUNT(*) FROM ( SELECT a.ID_DOCUMENTO , a.TITOLO , b.TITOLO MODELLO , a.DATA_INSERIMENTO ,  "
                                                         ...
                                                         + "            '<a href=\"'||decode(a.stato,'B','../restrict/ServletModulistica.do','../restrict/ServletModulisticaView.do')||'?ID='||a.id_documento||'&IDPD='||a.id_documento_padre||'&REV='||a.revisione ||decode( a.stato||amv_revisione.get_diritto_modifica('{Utente}',a.id_documento, a.revisione) , 'B1', '&rw=W' , '&rw=R' ) ||'&'||a.link||'&'||replace(amv_documento.get_modello(a.id_documento_padre,a.revisione),'/','&') ||'\"'||decode(a.stato,'B','',' target=\"_blank\"')||' >'||a.id_documento||'</a>' documento_link ,dr.NOTE note FROM AMV_DOCUMENTI a, AMV_DOCUMENTI b, AMV_DOCUMENTI_REVISIONI dr WHERE a.AUTORE = '{Utente}' AND '{Utente}' != 'GUEST' AND a.ID_DOCUMENTO_PADRE = b.ID_DOCUMENTO AND a.REVISIONE = b.REVISIONE AND a.ID_DOCUMENTO = dr.ID_DOCUMENTO AND a.REVISIONE = dr.REVISIONE AND a.id_documento_padre = nvl({s_MODELLO},a.id_documento_padre) AND instr(b.LINK,'iter=N inoltro=I') < 1 AND (a.titolo LIKE '%{s_TESTO}%' OR to_char(a.data_inserimento,'dd/mm/yyyy hh24:mi') LIKE '%{s_TESTO}%' OR amv_revisione.get_des_stato(a.STATO) LIKE '%{s_TESTO}%' OR b.TITOLO LIKE '%{s_TESTO}%' )  ) cnt " );
