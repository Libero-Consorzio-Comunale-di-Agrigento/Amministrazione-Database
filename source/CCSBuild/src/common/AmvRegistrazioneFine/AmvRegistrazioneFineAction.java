...
//AmvRegistrazioneFineAction: call children actions @1-F57796BA
        ...
        if ( page.getChild( "AmvDatiRegistrazione" ).isVisible() ) {
            page.getRequest().setAttribute("AmvDatiRegistrazioneParent",page);
            common.AmvDatiRegistrazione.AmvDatiRegistrazioneAction AmvDatiRegistrazione = new common.AmvDatiRegistrazione.AmvDatiRegistrazioneAction();
            result = result != null ? result : AmvDatiRegistrazione.perform( req, resp,  context );
            page.setCookies();
        }
        if (result == null) {
        ...
        if ( page.getChild( "RegistrazioneFine" ).isVisible() ) {
            page.getRequest().setAttribute("RegistrazioneFineParent",page);
            common.RegistrazioneFine.RegistrazioneFineAction RegistrazioneFine = new common.RegistrazioneFine.RegistrazioneFineAction();
            result = result != null ? result : RegistrazioneFine.perform( req, resp,  context );
            page.setCookies();
        }
...
//MESSAGGIO_RICHIESTA Grid: set where parameters @7-55248773
            try {
...
//MESSAGGIO_RICHIESTA Directory: getRowFieldByName @7-78F2D8E6
        public Object getRowFieldByName( String name, MESSAGGIO_RICHIESTARow row ) {
            Object value = null;
            if ( "MSG".equals(name) ) {
                value = row.getMSG();
            } else if ( "MSG_PROCEDURE".equals(name) ) {
                value = row.getMSG_PROCEDURE();
            }
            return value;
        }
