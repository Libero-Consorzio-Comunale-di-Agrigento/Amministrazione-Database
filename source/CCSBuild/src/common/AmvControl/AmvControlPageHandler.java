...
/*
javax.servlet.ServletContext context = e.getPage().getRequest().getSession().getServletContext();
String realContextPath = context.getRealPath("/");
org.apache.log4j.PropertyConfigurator.configure(realContextPath + "log4j.properties");
org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger("CCS");
*/
/*String urlH="";
if (e.getPage().getRequest().getQueryString() != null) {
   ...
   SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVURL", e.getPage().getRequest().getRequestURI() + "?" + urlH);
...
else
{
   SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVURL", e.getPage().getRequest().getRequestURI());
        ...
        if (e.getPage().getRequest().getQueryString() != null) {
            ...
            if (e.getPage().getRequest().getQueryString().indexOf("_h=")>=0) {
                String params[]=e.getPage().getRequest().getQueryString().split("&");
                for(int i=0;i<params.length;i++){
                    if(params[i].split("=")[0].equals("_h")){
                     queryString.append(StringUtils.decryptB64(params[i].split("=")[1].replaceAll("%3D","=").replaceAll("%2F","/")).replaceAll("\\?",""));
                    } else{
            ...
            {
                queryString.append(e.getPage().getRequest().getQueryString());
            }
            SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVURL", e.getPage().getRequest().getRequestURI() + "?" + queryString.toString());
//            SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVURL", e.getPage().getRequest().getRequestURI() + "?" + e.getPage().getRequest().getQueryString());
        } else {
            SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVURL", e.getPage().getRequest().getRequestURI());
        }
//log.info("URL_DECR Post:"+urlH);
...
if (e.getPage().getHttpGetParameter("MVRP")  != null) {
SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVRP", e.getPage().getHttpGetParameter("MVRP").toString().replace('*','&'));
}
...
if (e.getPage().getHttpGetParameter("MVPG")  != null) {
	current_mvpg = e.getPage().getHttpGetParameter("MVPG").toString();
}
...
if (e.getPage().getHttpGetParameter("MVPG")  != null) {
	SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVPC", e.getPage().getHttpGetParameter("MVPG"));
}
else {
//  CCS 2.2 .3
...
if (e.getPage().getHttpGetParameter("MVPD")  != null) {
   SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVPD",  e.getPage().getHttpGetParameter("MVPD") );
}
...
if (e.getPage().getHttpGetParameter("MVVC")  != null) {
	voce_corrente =	e.getPage().getHttpGetParameter("MVVC");
}
//log.info("URL_DECR voce_corrente"+voce_corrente);
...
if (voce_corrente != null) {
	...
	if (e.getPage().getHttpGetParameter("MVID")  != null) {
		SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVID", e.getPage().getHttpGetParameter("MVID"));
	} else {
		if (!pagina_corrente.equals(pagina_precedente)) {
			SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVID", null);
		}
	}
	if (e.getPage().getHttpGetParameter("MVID2")  != null) {
		SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVID2", e.getPage().getHttpGetParameter("MVID2"));
	} else {
		if (!pagina_corrente.equals(pagina_precedente)) {
 ...
 // Gestione variabili di sessione relative ai livelli di menu
  if (e.getPage().getHttpGetParameter("MVPD")  != null) {
  SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVL1",  e.getPage().getHttpGetParameter("MVL1") );
  SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVL2",  e.getPage().getHttpGetParameter("MVL2") );
  SessionStorage.getInstance(e.getPage().getRequest()).setAttribute("MVL3",  e.getPage().getHttpGetParameter("MVL3") );
 }
