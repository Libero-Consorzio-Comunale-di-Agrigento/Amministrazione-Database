//RECUPERO_LOGIN_CUSTOM DataSource @20-531B75CA
...
//End RECUPERO_LOGIN_CUSTOM DataSource

//class DataObject Header @20-FFEA32FF
public class RECUPERO_LOGIN_CUSTOMDataObject extends DS {
//End class DataObject Header

//attributes of DataObject @20-19DD80E8
    ...
    private RECUPERO_LOGIN_CUSTOMRow[] rows = new RECUPERO_LOGIN_CUSTOMRow[20];
...
//properties of DataObject @20-8AE152B2
    ...
    public RECUPERO_LOGIN_CUSTOMRow[] getRows() {
        return rows;
    }

    public void setRows(RECUPERO_LOGIN_CUSTOMRow[] rows) {
        this.rows = rows;
    }
...
//constructor @20-3968DD6E
    public RECUPERO_LOGIN_CUSTOMDataObject(Page page) {
        super(page);
    }
//End constructor
...
//load @20-483C126B
    boolean load() {
        ...
        command.setSql( "SELECT amvweb.get_preferenza ('Messaggio Recupero Password',  "
                    + "'{Modulo}') "
                    + "       || '<br/>' "
                    + "          LOSTMSGCUSTOM "
                    + "  FROM AD4_SERVIZI "
                    + " WHERE istanza = '{Istanza}' AND modulo = '{Modulo}'" );
        ...
        command.setCountSql( "SELECT COUNT(*) FROM ( SELECT amvweb.get_preferenza ('Messaggio Recupero Password',  "
                                                         + "            '{Modulo}') || '<br/>' LOSTMSGCUSTOM FROM AD4_SERVIZI WHERE istanza = '{Istanza}' AND modulo = '{Modulo}' ) cnt " );
...
//loadDataBind @20-84C5DB9E
        if ( records == null || ! records.hasMoreElements() ) {
            empty = true;
        } else {
            int counter = 0;
            while ( counter < rows.length && records.hasMoreElements() ) {
                RECUPERO_LOGIN_CUSTOMRow row = new RECUPERO_LOGIN_CUSTOMRow();
                DbRow record = (DbRow) records.nextElement();
                row.setLOSTMSGCUSTOM(Utils.convertToString(ds.parse(record.get("LOSTMSGCUSTOM"), row.getLOSTMSGCUSTOMField())));
                rows[counter++] = row;
            }
        }
//End loadDataBind
...
//getParameterByName @20-D3FA9408
