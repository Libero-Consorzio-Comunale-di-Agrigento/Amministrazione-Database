//AmvLoginSecureAction imports @1-C7BBE8C9
...
//End AmvLoginSecureAction imports

//AmvLoginSecureAction class @1-B7167EE2
public class AmvLoginSecureAction extends Action {

//End AmvLoginSecureAction class

//AmvLoginSecureAction: method perform @1-772C6836
    public String perform( HttpServletRequest req, HttpServletResponse resp, ServletContext context ) {
        ...
        page = new AmvLoginSecureModel( (CCSLocale) SessionStorage.getInstance( req ).getAttribute(Names.CCS_LOCALE_KEY));
        if (req.getAttribute(page.getName()+"Parent")==null) page.setCharacterEncoding("windows-1252");
        page.setResponse(resp);
        page.setRequest(req);
        res = ResourceBundle.getBundle("MessagesBundle", page.getCCSLocale().getLocale());
        req.setAttribute( "AmvLoginSecureModel", page );
...
//End AmvLoginSecureAction: method perform

//AmvLoginSecureAction: call children actions @1-FEB7DB88
...
//End AmvLoginSecureAction: call children actions
...
//LOGIN Record: init DataSource @2-A7331C6B
            if (!model.isAllowRead()) return;
            boolean isErrors = false;
            LOGINDataObject ds = new LOGINDataObject(page);
            ds.setComponent( model );
            ds.setUrlMVERR( page.getHttpGetParams().getParameter("MVERR") );
            ds.setSesUsername( SessionStorage.getInstance(req).getAttribute("username") );
            ds.setSesPassword( SessionStorage.getInstance(req).getAttribute("password") );
            ds.load();
            if ( ds.isEmpty() ) {
                model.setMode(com.codecharge.components.Record.INSERT_MODE);
            } else {
...
//LOGIN Record: bind @2-DAC73819
            public void bind(com.codecharge.components.Component model, LOGINRow row ) {
                ...
                model.getControl("JSLOGIN").setValue(row.getJSLOGIN());
                if ( this.valid ) {
                    model.getControl("u").setValue(row.getU());
                    model.getControl("p").setValue(row.getP());
                }
            }
//End LOGIN Record: bind

//LOGIN Record: getRowFieldByName @2-457AAA55
            public Object getRowFieldByName( String name, LOGINRow row ) {
                Object value = null;
                if ( "ACTION".equals(name) ) {
                    value = row.getACTION();
                } else if ( "ERRORE".equals(name) ) {
                    value = row.getERRORE();
                } else if ( "u".equals(name) ) {
                    value = row.getU();
                } else if ( "p".equals(name) ) {
                    value = row.getP();
                } else if ( "sa4ck".equals(name) ) {
                    value = row.getSa4ck();
                } else if ( "JSLOGIN".equals(name) ) {
                    value = row.getJSLOGIN();
                }
                return value;
            }
...
//LOGIN Record: method validate @2-A81E6769
        boolean validate() {
            model.fireOnValidateEvent();
            boolean isControlError = false;

            com.codecharge.components.TextBox u = (com.codecharge.components.TextBox) model.getChild( "u" );
            if (! u.validate()) { isControlError = true; }

            com.codecharge.components.TextBox p = (com.codecharge.components.TextBox) model.getChild( "p" );
            if (! p.validate()) { isControlError = true; }

            com.codecharge.components.Hidden sa4ck = (com.codecharge.components.Hidden) model.getChild( "sa4ck" );
            if (! sa4ck.validate()) { isControlError = true; }
...
//RECUPERO_LOGIN_CUSTOM Grid @20-CAF04914
    final class RECUPERO_LOGIN_CUSTOMClass {
        com.codecharge.components.Grid model;
        Event e;
//End RECUPERO_LOGIN_CUSTOM Grid

//RECUPERO_LOGIN_CUSTOM Grid: method perform @20-B48879D3
...
//RECUPERO_LOGIN_CUSTOM Grid: method read: head @20-A74AB037
        boolean read() {
            boolean isErrors = false;
...
//RECUPERO_LOGIN_CUSTOM Grid: method read: init @20-1362FFFA
            if ( ! model.allowRead ) return true;
...
//RECUPERO_LOGIN_CUSTOM Grid: set where parameters @20-B5B3F448
...
//RECUPERO_LOGIN_CUSTOM Grid: set grid properties @20-11C2D650
            
            ds.setPageNum( (model.getPage() < 1 ? 1 : model.getPage()) );
            ds.setPageSize( model.getFetchSize() );
//End RECUPERO_LOGIN_CUSTOM Grid: set grid properties

//RECUPERO_LOGIN_CUSTOM Grid: retrieve data @20-FE4E4327
            ds.load();
            if ( ! ds.isEmpty() ) {
...
//RECUPERO_LOGIN_CUSTOM Grid: check errors @20-B2B22DDA
            isErrors = ds.hasErrors();
            if ( isErrors ) model.addErrors( ds.getErrors() );
//End RECUPERO_LOGIN_CUSTOM Grid: check errors

//RECUPERO_LOGIN_CUSTOM Grid: method read: tail @20-F575E732
            return ( ! isErrors );
        }
//End RECUPERO_LOGIN_CUSTOM Grid: method read: tail

//RECUPERO_LOGIN_CUSTOM Grid: method bind @20-6E5B5B15
...
//RECUPERO_LOGIN_CUSTOM Directory: getRowFieldByName @20-8F30D31D
...
//RECUPERO_LOGIN_CUSTOM Grid: method validate @20-104025BA
        boolean validate() {
            return true;
        }
...
//AmvLoginSecure Page: method validate @1-104025BA
    boolean validate() {
        return true;
    }
//End AmvLoginSecure Page: method validate

//AmvLoginSecureAction Tail @1-FCB6E20C
}
//End AmvLoginSecureAction Tail
