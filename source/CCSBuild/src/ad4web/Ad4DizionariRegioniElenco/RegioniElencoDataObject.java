//RegioniElenco DataSource @8-194AE06A
...
//load @8-D6E56245
    boolean load() {
        ...
        command.setCountSql( "SELECT COUNT(*) FROM ( select regione ,denominazione ,id_regione ,utente_aggiornamento ,ad4_ccs.format_data_trunc(data_aggiornamento) data_aggiornamento ,  "
                                                         + "            '<a class=\"AFCDataLink\"' ||' href=\"' ||'AmvPage.do' ||'?'||'MVPG=Ad4DizionariProvinceElenco' ||'&'||'s_REGIONE='||regione ||'\"' ||'>' ||'<img border=0 src=\"../images/freccia_destra.gif\" title=\"vai a province\">' ||'</a>' ||'&'||'nbsp;' link_province from ad4_regioni where ( denominazione like upper('%'||'{s_REGIONE}')||'%' or regione like upper('{s_REGIONE}')||'%' )  ) cnt " );
