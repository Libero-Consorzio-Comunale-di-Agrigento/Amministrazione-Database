//DIRITTI_ACCESSO DataSource @146-187DFA6B
...
//load @146-FE893DD8
    boolean load() {
        ...
        command.setSql( "SELECT 'Gruppi a cui'||decode(UTEN.TIPO_UTENTE,  "
                    + "'U',' l''utente \"',' il gruppo \"')||UTEN.NOMINATIVO||'\" appartiene' NOMINATIVO "
                    + "  FROM UTENTI UTEN "
                    + " WHERE UTEN.UTENTE = decode(UTEN.TIPO_UTENTE, 'U','{AD4UTENTE}','G','{AD4GRUPPO}') "
                    + "   AND UTEN.TIPO_UTENTE like decode('{TIPOUTENTE}','M','%','{TIPOUTENTE}') "
                    + "UNION "
                    + "SELECT '<font color=\"#ff0000\">Selezionare un utente od un gruppo prima di accedere ai suoi Gruppi di Appartenenza!</font>' NOMINATIVO "
                    + "  FROM DUAL "
        ...
        command.setCountSql( "SELECT COUNT(*) FROM ( SELECT 'Gruppi a cui'||decode(UTEN.TIPO_UTENTE,  "
                                                         + "            'U',' l''utente \"',' il gruppo \"')||UTEN.NOMINATIVO||'\" appartiene' NOMINATIVO FROM UTENTI UTEN WHERE UTEN.UTENTE = decode(UTEN.TIPO_UTENTE, 'U','{AD4UTENTE}','G','{AD4GRUPPO}') AND UTEN.TIPO_UTENTE like decode('{TIPOUTENTE}','M','%','{TIPOUTENTE}') UNION SELECT '<font color=\"#ff0000\">Selezionare un utente od un gruppo prima di accedere ai suoi Gruppi di Appartenenza!</font>' NOMINATIVO FROM DUAL WHERE ('{AD4UTENTE}' is null AND '{TIPOUTENTE}' = 'U') OR ('{AD4GRUPPO}' is null AND '{TIPOUTENTE}' = 'G') ) cnt " );
