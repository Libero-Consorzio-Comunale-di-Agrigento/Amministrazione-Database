...
--changeset mturra:201901301231_213 runOnChange:true stripComments:false endDelimiter:/
...
/******************************************************************************
 ...
 17   07/10/2013 SN    Gestito blocco menu se password da modificare
 18   16/06/2020 SN    Se raggiungo il numero dei tentativi o non indicato numero di tentativi
...
BEGIN
-- blocco delle attivita  se deve cambiare password
-- rev.18 inizio
 ...
 if v_PWD_DA_MODIFICARE = 'SI' and nvl(registro_utility.leggi_stringa('PRODUCTS/AMV', 'Disabilita Menu se PWD da cambiare',0),'NO') = 'SI'
     ...
     then
     d_char :=  '<tr><td class="AFCErrorDataTD" width="100%">PRIMA DI PROCEDERE E'' NECESSARIO ESEGUIRE: <br><br>Modifica Password</tr></td>';
...
-- recupero proprieta immagine
       ...
       END IF;
       d_immagine := '<img src="../'||d_dir_upload||'/images/'||d_immagine||'" border="0" alt="'||d_nome||'" align="AbsMiddle" '||d_img_prop||'>&'||'nbsp;';
     END IF;
...
/******************************************************************************
 ...
 0    23/07/2004 AO     Prima emissione.
 1    30/03/2005 AO     Ordinamento documenti sui campi sequenza e titolo
 2    01/12/2005 AO     Aggiunto id=ril_nomerilevanza per div di ciascuna
                        rilevanza. Aggiunto per specifica personalizzazione
...
-- Rev.2
--   d_row_start := '<div class="AFC'||d_sfx||'"><table width="100%" cellpadding="3"  cellspacing="0" class="AFC'||d_sfx||'FormTable">';
   d_row_start := '<div class="AFC'||d_sfx||'" id="ril_'||d_nome||'"><table width="100%" cellpadding="3"  cellspacing="0" class="AFC'||d_sfx||'FormTable">';
-- Rev.2: fine.
   d_row_end   := '</table></div>';
   d_row_sep  := '<tr><td class="AFC'||d_sfx||'SeparatorTD" width="100%"><img src="../common/images/AMV/blank_separator.gif"></tr></td>';
...
BEGIN
   ...
   IF d_abstract IS NOT NULL THEN
     ...
     d_char := '&'||'nbsp;(<a href="../common/AmvDocumentoInfo.do?MVVC=amvdocui&'||'ID='||p_id_documento||'&'||'REV='||d_revisione||'&'||'MVPD=0&'||'MVTD='||d_id_tipologia||'&'||'MVSZ='||
...
IF d_aggiorna = -1 THEN
   ...
   ELSE
     ...
     d_parametri_stampa := d_parametri_stampa||'&'||'cr='||p_codice_richiesta||'&'||'rw=R';
     d_return_message := '<a target="_blank" href="../restrict/ServletModulisticaPrint.do?'||d_parametri_stampa||'">Stampa&'||'nbsp;il&'||'nbsp;modulo&'||'nbsp;<img src="../common/images/AMV/printer.gif" border="0"></a>';
      d_cronologia :=  CHR(10)||'- '||TO_CHAR(SYSDATE,'dd/mm/yyyy hh:mi')||
                    ' - '||P_UTENTE||'- Inoltro';
   END IF;
   INSERT INTO AMV_DOCUMENTI_REVISIONI (
...
BEGIN
   ...
   ELSE
      IF p_importanza = 'HL' THEN
        d_url := NVL(d_img,'<span class="AFCCenterTitle">'||p_titolo||'</span>');
      ELSE
