...
--changeset mturra:201901301231_174 runOnChange:true stripComments:false
...
/******************************************************************************
 ...
 010  29/06/2016 Sneg   Modificata view modify x consentire creazione di viste con testo più lungo di 32000
 011  26/09/2019 SNeg   Gestire la upper x la ricerca del testo della vista
 012  26/09/2019 SNeg   Controllo su presenza di operatori insiemistici corretto
 013  25/01/2021 SNeg   Creare la vista se operatore insiemistico ma non esiste e mettere la force Bug #47675
*******************************************************************************/
s_revisione_body AFC.t_revision := '013';
...
END SQL_EXECUTE;
 PROCEDURE VIEW_MODIFY (nome_vista    VARCHAR2,
   ...
   ******************************************************************************/
   ...
   v_controllo_nome varchar2(100); -- rev. 13
   ...
   BEGIN
      ...
      THEN
         LOOP
            ...
            IF nvl(v_pos_operatore,0) = 0 -- rev. 12 controllo con 0 e non con null
            THEN
            ...
            THEN
               ...
               text_table (v_num_riga) :=
                  'create or replace force view ' || p_value || ' as ';
            END IF;
   ...
   BEGIN
      ...
      THEN
         v_num_riga := v_num_riga + 1;
         text_table (v_num_riga) :=
            'create or replace force view ' || nome_vista || ' as ';
         v_num_riga := v_num_riga + 1;
      ...
      THEN
        ...
        <<estrazione_testo>>
         BEGIN
         -- rev. 13 inizio
         -- controlla se esiste la vista, in quel caso prende il testo
         -- altrimenti genera no_data_found e la crea
            select view_name
              into v_controllo_nome
              from user_views
             where view_name = upper(nome_vista);
             -- rev. 13 fine
            get_view_text (upper(nome_vista)); -- rev. 11
