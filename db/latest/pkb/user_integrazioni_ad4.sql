...
--changeset snegroni:20190604_1228 runOnChange:true stripComments:false


CREATE OR REPLACE PACKAGE BODY USER_INTEGRAZIONI_AD4
IS
/******************************************************************************
 NOME:        USER_INTEGRAZIONI_AD4
 ...
 Refernzia il package ADMIN_AD4
REVISIONI:
...
-------------------------------------------------------------------
            -- TABLE - VIEW
            FOR j IN NVL (admin_ad4.TabTVDB.FIRST, 1) .. NVL (admin_ad4.TabTVDB.LAST, 0)
            LOOP
               gestione_sinonimo_grant(p_drop_create => p_drop_create, p_user => p_user, p_object => admin_ad4.TabTVDB (j).object_name, p_synonym_name => '', p_privilege => p_privilege, p_error => d_error, p_owner => p_owner);
            END LOOP;
            -- FUNCTION / PACKAGE
            FOR j IN NVL (admin_ad4.TabPDB.FIRST, 1) .. NVL (admin_ad4.TabPDB.LAST, 0)
            LOOP
               gestione_sinonimo_grant(p_drop_create, p_user, admin_ad4.TabPDB (j).object_name, '', 'execute', d_error, p_owner);
            END LOOP;
...
-------------------------------------------------------------------
            -- TABLE - VIEW
            FOR j IN NVL (admin_ad4.TabTVCM.FIRST, 1) .. NVL (admin_ad4.TabTVCM.LAST, 0)
            LOOP
               gestione_sinonimo_grant(p_drop_create, p_user, admin_ad4.TabTVCM (j).object_name, '', p_privilege, d_error, p_owner);
            END LOOP;
            -- FUNCTION / PACKAGE
            FOR j IN NVL (admin_ad4.TabPCM.FIRST, 1) .. NVL (admin_ad4.TabPCM.LAST, 0)
            LOOP
               gestione_sinonimo_grant(p_drop_create, p_user, admin_ad4.TabPCM (j).object_name, '', 'execute', d_error, p_owner);
            END LOOP;
         END IF;
         IF p_what IN ('BS', 'ALL')
         THEN
...
-------------------------------------------------------------------
            -- TABLE - VIEW
            FOR j IN NVL (admin_ad4.TabTVBS.FIRST, 1) .. NVL (admin_ad4.TabTVBS.LAST, 0)
            LOOP
               gestione_sinonimo_grant(p_drop_create, p_user, admin_ad4.TabTVBS (j).object_name, '', p_privilege, d_error, p_owner);
            END LOOP;
            -- FUNCTION / PACKAGE
            FOR j IN NVL (admin_ad4.TabPBS.FIRST, 1) .. NVL (admin_ad4.TabPBS.LAST, 0)
            LOOP
               gestione_sinonimo_grant(p_drop_create, p_user, admin_ad4.TabPBS (j).object_name, '', 'execute', d_error, p_owner);
            END LOOP;
         END IF;
...
-------------------------------------------------------------------
            -- TABLE - VIEW
            FOR j IN NVL (admin_ad4.TabTVASL.FIRST, 1) .. NVL (admin_ad4.TabTVASL.LAST, 0)
            LOOP
               gestione_sinonimo_grant(p_drop_create, p_user, admin_ad4.TabTVASL (j).object_name, '', p_privilege, d_error, p_owner);
            END LOOP;
         END IF;
...
-------------------------------------------------------------------
            -- FUNCTION / PACKAGE
            FOR j IN NVL (admin_ad4.TabPXDK.FIRST, 1) .. NVL (admin_ad4.TabPXDK.LAST, 0)
            LOOP
               gestione_sinonimo_grant(p_drop_create, p_user, admin_ad4.TabPXDK (j).object_name, '', 'execute', d_error, p_owner);
            END LOOP;
         END IF;
      END IF;
      declare
...
BEGIN
  select owner
    ...
    from all_objects
   where object_name = 'USER_INTEGRAZIONI_AD4'
