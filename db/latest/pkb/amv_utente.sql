...
--changeset mturra:201901301231_121 runOnChange:true stripComments:false


CREATE OR REPLACE PACKAGE BODY Amv_Utente AS


d_revisione VARCHAR2(30) := '21   18/11/2020';
...
/******************************************************************************
 ...
 1    07/11/2005 AO     Aggiunti parametri per registrazione recapito
 2    20/12/2006 AO     Aggiunto controllo su data di nascita
 3    23/01/2007 AO     Aggiunta chiamata procedure di pre-validazione
 4    07/05/2007 AO     Corretta chiamata procedure di pre-validazione
******************************************************************************/

d_utente          VARCHAR2(8);
d_mezza_password  VARCHAR2(4);
d_indirizzo          VARCHAR2(40);
d_indirizzo_web   VARCHAR2(40) := p_indirizzo_web;
d_telefono           VARCHAR2(40) := p_telefono;
d_fax              VARCHAR2(40) := p_fax;
d_comune          NUMBER(3) := p_comune;
d_provincia          NUMBER(3) := p_provincia;
d_cap              VARCHAR2(5) := p_cap;
d_pre_valida      NUMBER(1) := 0;

BEGIN
   IF NOT (to_date(p_data_nascita,'dd/mm/yyyy') BETWEEN to_date('01/01/1900','dd/mm/yyyy') AND to_date('30/12/2010','dd/mm/yyyy')) THEN
      raise_application_error(-20999,'Data di nascita non valida');
   END IF;
      -- Verifica esistenza procedure di pre-validazione specifica
         BEGIN
               SELECT 1
              INTO d_pre_valida
              FROM OBJ
                WHERE OBJECT_NAME = 'PRE_VALIDA_ABILITAZIONE'
                  AND OBJECT_TYPE = 'PROCEDURE';
         EXCEPTION WHEN NO_DATA_FOUND THEN
            NULL;
         END;
         IF d_pre_valida = 1 THEN
            BEGIN
      -- Lancio procedure di validazione specifica utilizzando package SI4
          Si4.SQL_EXECUTE('begin PRE_VALIDA_ABILITAZIONE('''||replace(p_nominativo,'''','''''')
                                                                 ||''','''||replace(p_cognome,'''','''''')
                                                                 ||''','''||p_nome
                                                                 ||''','''||p_codice_fiscale
                                                                 ||''','||nvl(to_char(p_comune),'''''')
                                                                 ||','||nvl(to_char(p_provincia),'''''')
                                                                 ||','''||p_cap
                                                                 ||''','''||p_via
                                                                 ||''','''||replace(p_indirizzo,'''','''''')
                                                                 ||''','''||p_numero
                                                                 ||''','||nvl(to_char(p_comune_nas),'''''')
                                                                 ||','||nvl(to_char(p_provincia_nas),'''''')
                                                                 ||','''||p_data_nascita
                                                                 ||''','''||p_sesso
                                                                 ||''','''||p_indirizzo_web
                                                                 ||''','''||p_telefono
   ...
   IF p_indirizzo IS NOT NULL THEN
      d_indirizzo := substr(p_via||' '||p_indirizzo||', '||p_numero,1,40);
   END IF;

   BEGIN -- Registrazione utente stato Sospeso
      AD4_UTENTE.REGISTRA_WEB(d_utente, d_mezza_password,UPPER(rtrim(p_nominativo,' ')), UPPER(rtrim(p_cognome,' ')), UPPER(rtrim(p_nome,' ')), UPPER(p_codice_fiscale), d_comune, d_provincia, d_cap, d_indirizzo, p_comune_nas, p_provincia_nas, p_data_nascita, p_sesso, d_indirizzo_web, d_telefono, d_fax);
      p_utente := d_utente;
...
BEGIN
   IF p_indirizzo IS NOT NULL THEN
      d_indirizzo := p_via||' '||p_indirizzo||', '||p_numero;
   END IF;

   BEGIN -- Registrazione utente stato Sospeso
      AD4_UTENTE.REGISTRA_WEB(d_utente, d_mezza_password,UPPER(rtrim(p_nominativo,' ')), UPPER(rtrim(p_cognome,' ')), UPPER(rtrim(p_nome,' ')), null, d_comune, d_provincia, d_cap, d_indirizzo, null, null, null, null, d_indirizzo_web, d_telefono, d_fax);
      p_utente := d_utente;
      p_mezza_password := d_mezza_password;
   END;
END REGISTRA_UTENTE_LITE;
...
BEGIN
   ...
   --IF d_required IS NOT NULL THEN
      IF p_richiesta IS NULL THEN -- Crea nuova richiesta solo se non e gia presente una richiesta
         ...
         p_richiesta := d_richiesta;
         AD4_UTENTE.AGGIORNA_WEB(d_utente, 0, 0, 'NO', 'NO', 'NO', 'NO', 'NO', 'Y');
      END IF;
   --ELSE
   NULL;
...
/******************************************************************************
 ...
 3    19/09/2006 AO     Corretta chiamata a AD4_UTENTE.SET_PASSWORD con 2
                        parametri (mancavano apici).
******************************************************************************/
d_nominativo     VARCHAR2(40);
d_password       VARCHAR2(40);
d_ldap           NUMBER := 0;
...
/******************************************************************************
 NOME:        RECUPERA_LOGIN
 DESCRIZIONE: Consente il recupero della password da applicativo
 PARAMETRI:
 ECCEZIONI:
 ANNOTAZIONI: -
 REVISIONI:
 Rev. Data       Autore Descrizione
 ---- ---------- ------ ------------------------------------------------------
 20   05/04/2019 SN     Verifica della mail maiuscola  Bug #34221
******************************************************************************/
d_soggetto        NUMBER(8);
d_nome            VARCHAR2(50);
d_email           VARCHAR2(100):= upper(p_email); --rev. 20 Bug #34221
d_esiste          NUMBER(1);
d_utente          VARCHAR2(8);
d_nominativo      VARCHAR2(40);
...
BEGIN
   ...
   END IF;
   IF d_email IS NULL THEN
      RAISE_APPLICATION_ERROR(-20999,'Email Utente obbligatoria.');
   END IF;
   d_nome := UPPER(p_cognome)||'  '||UPPER(p_nome);
   -- Verifica se procedura di recupero e full (richiede dati di nascita e codice fiscale)
   IF (amvweb.get_preferenza('Recupero Password Full',p_modulo) = 'SI') then
      IF p_codice_fiscale IS NULL THEN
         RAISE_APPLICATION_ERROR(-20999,'Codice Fiscale obbligatorio.');
      END IF;
      IF p_comune_nas IS NULL or p_provincia_nas IS NULL or p_data_nascita IS NULL THEN
         RAISE_APPLICATION_ERROR(-20999,'Dati di Nascita obbligatorio.');
      END IF;
      IF p_sesso IS NULL THEN
         RAISE_APPLICATION_ERROR(-20999,'Campo sesso obbligatorio per verifica del codice fiscale');
      END IF;
      IF ad4_codice_fiscale.controllo(p_codice_fiscale,p_sesso,p_comune_nas,p_provincia_nas,p_data_nascita)<0 THEN
         RAISE_APPLICATION_ERROR(-20999,'Verifica Codice Fiscale fallita.');
      END IF;
   END IF;
   -- Selezione mittente da campo MAIL_NOTIFICHE del servizio
   BEGIN
   ...
   EXCEPTION
      WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR(-20999,'Indirizzo del mittente non recuperabile');
   END;
   ...
   ELSE
      ...
      /* Verifico univocita del soggetto*/
         SELECT soggetto
           ...
           FROM ad4_soggetti
          WHERE nome = d_nome
            AND upper(indirizzo_web) = d_email;--rev. 20 Bug #34221
      EXCEPTION
      ...
      END;
   END IF;
   /* Determino il nominativo utente */
   BEGIN
   ...
   END;
   /* Verifico diritto di accesso utente */
   BEGIN
      SELECT uten.utente
        INTO d_utente
        FROM ad4_utenti uten, ad4_utenti_soggetti utso, ad4_diritti_accesso diac
       WHERE utso.soggetto = d_soggetto
         AND uten.utente = utso.utente
   ...
   EXCEPTION
      WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR(-20999,'Utente non abilitato al servizio');
   END;
   ...
   EXCEPTION
      WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR(-20999,'Password scaduta o utente non abilitato al cambio password');
   END;
   /* Genero codice di controllo */
    ...
    UPDATE ad4_utenti
     ...
     d_send := Amvweb.send_msg(d_mittente,
                          p_email,
                         'Informazioni Account',
                         d_string||CHR(10)||
                         'Nominativo: '||d_nominativo||CHR(10)||
                         'Link:'||d_link||d_codice,
                          NVL(LOWER(ad4_registro_pac.get_ad4_string('RICHIESTA_ABILITAZIONE/MAIL/CIM', 'Tag', 'AD4WEB', 'AD4')),'mail')
                         );
...
/******************************************************************************
 NOME:        GET_MESSAGGIO_RICHIESTA.
 DESCRIZIONE: Restituisce il messaggio relativo alla conclusione della registrazione

 PARAMETRI:   P_ID_RICHIESTA    VARCHAR2  ID della richiesta

 RITORNA:     VARCHAR2  : Messaggio
 ANNOTAZIONI: -
 REVISIONI:
 Rev. Data       Autore Descrizione
 ---- ---------- ------ ------------------------------------------------------
 0    27/06/2003 AO     Prima emissione.
 1    18/11/2005 AO     Utilizzo del valore 'null' della preferenza per restituire
                        stringa vuota
******************************************************************************/
     p_id_richiesta IN NUMBER
) RETURN VARCHAR2 IS
...
BEGIN
   ...
   ELSE
      ...
      END IF;
   END IF;
   IF UPPER(d_messaggio) = 'NULL' THEN
      d_messaggio := '';
   END IF;
   RETURN d_messaggio;
...
/******************************************************************************
 ...
 2    19/01/2007 AO     Modifica per validita di utenti senza diritti di accesso
                        ma per i quali esiste richiesta respinte
******************************************************************************/
( p_utente          VARCHAR2
, p_amministratore  VARCHAR2
...
/******************************************************************************
 ...
 ---- ---------- ------ ------------------------------------------------------
 0    27/06/2003 AO     Prima emissione.
 21   18/11/2020 SN     Verifica durata password e numero tentativi su tutti i diritti di accesso Bug #46185
******************************************************************************/
( p_utente          VARCHAR2
, p_modulo          VARCHAR2
, p_istanza            VARCHAR2
...
BEGIN
   SELECT
       'Richiesta <strong>'||DECODE(RIAB.stato,'C','da prendere in carico','A','approvata','F','fallita','R','respinta','in stato non definito')||'</strong>.<br>'||
       '<strong>Notifica</strong> '||
      DECODE(RIAB.indirizzo_notifica,NULL,'','a '||LOWER(RIAB.indirizzo_notifica||' '))||
      'via '||NVL(LOWER(NVL(RIAB.tipo_notifica, serv.tipo_notifica)),'<em>tipo notifica non definito</em>')||
      '<strong>'||DECODE(RIAB.notificata,'S',' avvenuta correttamente','N',' non avvenuta','F',' fallita')||'</strong>.'||
       DECODE(RIAB.note_notifica,NULL,'','<br>Precisazione messaggio: '||RIAB.note_notifica)||
       DECODE(RIAB.note,NULL,'','<br>Note: '||RIAB.note)
  INTO d_stringa
  FROM ad4_RICHIESTE_ABILITAZIONE RIAB, ad4_servizi serv
 WHERE RIAB.id_richiesta = p_id_richiesta
   AND serv.Istanza = riab.Istanza
   AND serv.modulo = riab.modulo
   ;
   RETURN d_stringa;
EXCEPTION WHEN OTHERS THEN
   RETURN '';
...
IS
 d_tab                   VARCHAR2(30000);
 d_mail_notificate       VARCHAR2(200) := '../amvadm/AdmRichieste.do?TIPO_NOTIFICA=MAIL'
                                          ||'&'||'NOTIFICATA=S'
                                          ||'&'||'MVAV='||p_MVAV
                                          ||'&'||'IST='||p_IST
                                          ||'&'||'MOD='||p_MOD
 ...
 d_mail_non_notificate   VARCHAR2(200) := '../amvadm/AdmRichieste.do?TIPO_NOTIFICA=MAIL'
                                          ||'&'||'NOTIFICATA=F'
                                          ||'&'||'MVAV='||p_MVAV
                                          ||'&'||'IST='||p_IST
                                          ||'&'||'MOD='||p_MOD
                                          ||'&'||'STATO='||p_STATO;
 d_posta_notificate      VARCHAR2(200) := '../amvadm/AdmRichieste.do?TIPO_NOTIFICA=POSTA'
                                          ||'&'||'NOTIFICATA=S'
                                          ||'&'||'MVAV='||p_MVAV
                                          ||'&'||'IST='||p_IST
                                          ||'&'||'MOD='||p_MOD
                                          ||'&'||'STATO='||p_STATO;
 d_posta_non_notificate  VARCHAR2(200) := '../amvadm/AdmRichieste.do?TIPO_NOTIFICA=POSTA'
                                          ||'&'||'NOTIFICATA=N'
                                          ||'&'||'MVAV='||p_MVAV
                                          ||'&'||'IST='||p_IST
                                          ||'&'||'MOD='||p_MOD
                                          ||'&'||'STATO='||p_STATO;
 d_notificata            VARCHAR2(50);
 d_tipo_notifica         VARCHAR2(50);
BEGIN
   d_notificata    := NVL(p_notificata,'S');
   d_tipo_notifica := NVL(p_tipo_notifica,'MAIL');
   IF d_notificata != 'S' AND d_notificata != 'N' AND d_notificata != 'F' THEN
      d_notificata := 'S';
   END IF;
   IF d_tipo_notifica != 'MAIL' AND d_tipo_notifica != 'POSTA' THEN
      d_tipo_notifica := 'MAIL';
   END IF;
   IF d_notificata = 'S' AND d_tipo_notifica = 'MAIL' THEN
        d_tab := d_tab
                 ||'<td>'
                 ||Afc_Html.tab_folder('Mail inviate',d_mail_notificate,'S')
                 ||'</td>'
                 ||'<td>'
                 ||Afc_Html.tab_folder('Mail non inviate',d_mail_non_notificate,'N')
                 ||'</td>'
                 ||'<td>'
                 ||Afc_Html.tab_folder('Posta notificata',d_posta_notificate,'N')
                 ||'</td>'
                 ||'<td>'
                 ||Afc_Html.tab_folder('Posta da notificare',d_posta_non_notificate,'N')
                 ||'</td>'
                 ;
   END IF;
   IF d_notificata = 'F' AND d_tipo_notifica = 'MAIL' THEN
        d_tab := d_tab
                 ||'<td>'
                 ||Afc_Html.tab_folder('Mail inviate',d_mail_notificate,'N')
                 ||'</td>'
                 ||'<td>'
                 ||Afc_Html.tab_folder('Mail non inviate',d_mail_non_notificate,'S')
                 ||'</td>'
                 ||'<td>'
                 ||Afc_Html.tab_folder('Posta notificata',d_posta_notificate,'N')
                 ||'</td>'
                 ...
                 ;
   END IF;
   IF d_notificata = 'S' AND d_tipo_notifica = 'POSTA' THEN
        d_tab := d_tab
                 ||'<td>'
                 ||Afc_Html.tab_folder('Mail inviate',d_mail_notificate,'N')
                 ||'</td>'
                 ...
                 ||'</td>'
                 ||'<td>'
                 ||Afc_Html.tab_folder('Posta notificata',d_posta_notificate,'S')
                 ||'</td>'
                 ||'<td>'
                 ||Afc_Html.tab_folder('Posta da notificare',d_posta_non_notificate,'N')
                 ||'</td>'
                 ;
   END IF;
   IF d_notificata = 'N' AND d_tipo_notifica = 'POSTA' THEN
        d_tab := d_tab
                 ||'<td>'
                 ||Afc_Html.tab_folder('Mail inviate',d_mail_notificate,'N')
                 ||'</td>'
                 ...
                 ||'</td>'
                 ||'<td>'
                 ||Afc_Html.tab_folder('Posta da notificare',d_posta_non_notificate,'S')
                 ||'</td>'
                 ;
   END IF;
   RETURN '<table border="0" cellpadding="0" cellspacing="0"><tr>'||d_tab||'</tr></table>';
END;
...
IS
  d_url  VARCHAR2(5000) := '/jasperserver/jasperservlet?'
                          ||'PROJECT='||REPLACE(p_contesto,'/','')
                          ||'&'||'amp;REPORT=richieste_abilitazione_group'
                          ||'&'||'amp;ID_RICHIESTA='
                          ||'&'||'amp;CTRL=S';
  d_button VARCHAR2(5000);
BEGIN
   d_url := d_url
            ||'&'||'amp;MODULO='||p_modulo
            ||'&'||'amp;ISTANZA='||p_istanza;
   d_button := '<input class="AFCButton" '
             ||'id="Button1" type="button" value="Stampa lettere" name="Button1" '
             ||'onclick="window.open('''||d_url||''',''Notifiche'', ''resizable=yes,scrollbars=yes,status=yes'');">';
   RETURN d_button;
END;
...
IS
  d_link  VARCHAR2(5000) := '<a class="AFCDataLink" href="/jasperserver/jasperservlet?';
BEGIN
  d_link := d_link
             ||'&'||'PROJECT='||REPLACE(p_contesto,'/','')
             ||'&'||'REPORT=richieste_abilitazione_group'
             ||'&'||'ID_RICHIESTA='||p_id_richiesta
             ||'&'||'CTRL='||p_ctrl
             ||'&'||'MODULO='||p_modulo
             ||'&'||'ISTANZA='||p_istanza
             ||'&'||'conn=jdbc/ad4'
             ||'" target="_blank"><img src="../common/images/AMV/printer.gif" border="0" alt="Stampa report" align="AbsMiddle"/></a>';
  RETURN d_link;
END;
...
BEGIN
  d_url := d_url
             ...
             ||'&'||'ISTANZA='||p_istanza;
   d_button := '<input class="AFCButton" '
             ||'id="Button1" type="button" value="Stampa" name="Button1" '
             ||'onclick="window.open('''||d_url||''',''Notifiche'', ''resizable=yes,scrollbars=yes,status=yes'');">';
...
IS
BEGIN
   UPDATE ad4_richieste_abilitazione
         SET note_notifica = NULL
       WHERE tipo_notifica = 'POSTA'
         AND notificata = 'S';
END;

PROCEDURE SET_TIPO_NOTIFICA_POSTA(p_id_richiesta IN NUMBER)
IS
 d_email        VARCHAR2(200) := NULL;
 d_tipo_notifica_servizio  VARCHAR2(10);
BEGIN
  SELECT AD4_SOGGETTO.GET_INDIRIZZO_WEB(AD4_UTENTE.GET_SOGGETTO(r.utente)) email, s.tipo_notifica
    INTO d_email, d_tipo_notifica_servizio
    FROM ad4_richieste_abilitazione r, ad4_servizi s
   WHERE id_richiesta = p_id_richiesta
     AND r.modulo = s.modulo
     AND r.istanza = s.istanza;

  IF d_email IS NULL OR d_tipo_notifica_servizio = 'POSTA' THEN
     UPDATE ad4_richieste_abilitazione
        SET tipo_notifica = 'POSTA'
           ,notificata    = 'N'
      WHERE id_richiesta = p_id_richiesta;
  END IF;
END;
