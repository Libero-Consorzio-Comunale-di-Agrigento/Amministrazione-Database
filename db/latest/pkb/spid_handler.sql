...
--changeset mturra:201901301231_184 runOnChange:true stripComments:false
...
CREATE OR REPLACE package body spid_handler is
    ...
    022 09/05/2022 AD Aggiunta funzione di verifica del soggetto per personalizzazione San Martino
     richiede creazione della seguente chiave di registro
     PRODUCTS/SPIDHANDLER/<profilo> TipoAccesso L
     ed eventualmente
     PRODUCTS/SPIDHANDLER/<profilo> GiorniUltimoAccesso x
     dove x rappresenta il numero di giorni da scalare dalla data corrente per
     valutare il riutilizzo del soggetto e dell'utente

    ******************************************************************************/
    s_revisione_body AFC.t_revision := '022';
...
******************************************************************************/
    ...
    function check_soggetto(p_soggetto in number, p_profilo in varchar2 default null) return number is
        d_tipo_accesso varchar2(2000);
        ret_val number(1);
        D_ULTIMO_ACCESSO DATE;
        d_giorni_accesso number;
    begin
        DBMS_OUTPUT.PUT_LINE('Verifico profilo '||p_profilo);
        if p_profilo is not null then
            d_tipo_accesso := REGISTRO_UTILITY.LEGGI_STRINGA('PRODUCTS/SPIDHANDLER/'||UPPER(P_PROFILO),'TipoAccesso',0);
            DBMS_OUTPUT.PUT_LINE('Tipo accesso '||d_tipo_accesso);
            if nvl(d_tipo_accesso,'X') = 'L' then -- profilo di accesso interno
            /* SE IL SOGGETTO E' LEGATO AD UN UTENTE CHE NON HA FATTO ACCESSO DA ALMENO 12 MESI VA BENE ANCHE SE E' UN COMPONENTE */
                BEGIN
                    SELECT ULTIMO_TENTATIVO
                    INTO D_ULTIMO_ACCESSO
                    FROM UTENTI U, UTENTI_SOGGETTI US
                    WHERE US.UTENTE = U.uTENTE
                      AND US.SOGGETTO = P_SOGGETTO;
                EXCEPTION WHEN NO_DATA_FOUND THEN
                    D_ULTIMO_ACCESSO := NULL;
                END;
                dbms_output.put_line('Trovato utente con ultimo accesso '||d_ultimo_accesso);
                IF D_ULTIMO_ACCESSO IS NULL THEN -- il soggetto è legato ad un utente che non ha mai fatto accesso o non esiste
                    ret_val := 0; -- forzo il sistema a riutilizzare eventuali soggetti già presenti nel sistema
                else
                    d_giorni_accesso := to_number(nvl(REGISTRO_UTILITY.LEGGI_STRINGA('PRODUCTS/SPIDHANDLER/'||UPPER(P_PROFILO),'GiorniUltimoAccesso',0),'365'));
                    if D_ULTIMO_ACCESSO <= sysdate - d_giorni_accesso then
                        dbms_output.put_line('Non accede da più di '||d_giorni_accesso||' giorni. Lo riuso');
                        ret_val := 0;
                    else
                        dbms_output.put_line('E'' utente in uso lo scarto');
                        ret_val := 1;
                    end if;
                end if;
            else
                ret_val := soggetto.is_soggetto_componente(p_soggetto);
            end if;
        else
            ret_val := soggetto.is_soggetto_componente(p_soggetto);
        END IF;
            ...
            then
                begin
                    ...
                    where codice_fiscale = p_codice_fiscale
                      -- 12/04/2022 modifica alla funzione di verifica dei soggetti
                      AND spid_handler.check_soggetto(SOGGETTO,p_profilo) = 0
                    --
                    ;
                exception when no_data_found then
                    null;
...
-- 27/06/2019 gestione di più soggetti con stesso codice fiscale
                    ...
                    where codice_fiscale = p_codice_fiscale
                      ...
                      AND spid_handler.check_soggetto(SOGGETTO,p_profilo) = 0
                      and data_aggiornamento = ( select max(data_aggiornamento) from soggetti s2 where s1.codice_fiscale = s2.codice_fiscale and spid_handler.check_soggetto(s2.SOGGETTO,p_profilo) = 0);
--
                end;
                if d_soggetto is not null then -- soggetto già presente in anagrafica cerco se ha un utente con nominativo uguale al CF
                    ...
                    -- verifico se accesso su profilo intranet by spid per soggetto già legato ad un utente
                    if nvl(REGISTRO_UTILITY.LEGGI_STRINGA('PRODUCTS/SPIDHANDLER/'||UPPER(P_PROFILO),'TipoAccesso',0),'X') = 'L' then
                        declare
                            d_statement varchar2(4000);
                        begin
                            -- cerco se per accesso locale esiste un utente abbianato ad un soggetto con codice fiscale congruente che è in struttura
                            d_statement :='select s.ni
                            from utenti u, utenti_soggetti us, as4_anagrafe_soggetti s
                            where u.utente = us.utente
                              and us.soggetto = s.ni
                              and trunc(sysdate) between s.dal and nvl(s.al,to_date(3333333,''j''))
                              and soggetto.is_soggetto_componente(s.ni) = 1
                              and s.codice_fiscale = ' || p_codice_fiscale
                            ;
                            execute immediate d_statement INTO d_sogg_interno;

                        exception when no_data_found        then
                            d_sogg_interno := null;
                        end;
                        if d_sogg_interno is not null then
                            raise_application_error(-20999,'Accesso SPID non ammesso per utenti interni');
                        end if;
                    end if;
                    D_DATA_NAS      := CODICE_FISCALE.GET_DATA_NAS(P_CODICE_FISCALE);
                    D_COMUNE_NAS    := CODICE_FISCALE.GET_COMUNE_NAS(P_CODICE_FISCALE);
