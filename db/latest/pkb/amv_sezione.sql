...
--changeset mturra:201901301231_120 runOnChange:true stripComments:false
...
-- riga della sezione corrente
   ...
   IF d_immagine IS NOT NULL THEN
      d_immagine := '<img src="../'||d_dir_upload||'/images/'||d_immagine||'" border="0" alt="'||d_nome||'" align="AbsMiddle">&'||'nbsp;';
   END IF;
...
/******************************************************************************
 ...
 ---- ---------- ------ ------------------------------------------------------
 0    20/07/2004 AO     Prima emissione.
 1    27/01/2006 AO     Eliminato messaggio nessun documento disponibile se sezione vuota
 2    26/03/2007 AO     Aggiunto utilizzo function is_empty
******************************************************************************/
d_row_start       VARCHAR2(1000);
d_row_int         VARCHAR2(1000);
d_row_end         VARCHAR2(1000);
...
-- Assegnazione stile
   d_sfx := 'Center';
-- righe di apertura e chiusura
   d_row_start := '<div class="AFC'||d_sfx||'"><table width="100%" cellpadding="0"  cellspacing="0" class="AFC'||d_sfx||'Table">';
   d_row_end   := '</table></div>';
...
-- ciclo su sottosezioni tipologie rilevanze e documenti
   FOR c IN (
      ...
      and is_empty(id_sezione, p_utente) = 0
...
/******************************************************************************
 ...
 ---- ---------- ------ ------------------------------------------------------
 0    20/07/2004 AO     Prima emissione.
 1    26/03/2007 AO     Aggiunto utilizzo function is_empty
 6   07/10/2013 SN    Gestito blocco menu se password da modificare
 7   16/06/2020 SN    Se raggiungo il numero dei tentativi o non indicato numero di tentativi
                       bloccare menu. Feature #43041
******************************************************************************/
d_row_start     VARCHAR2(1000);
d_row_int       VARCHAR2(1000);
d_row_end       VARCHAR2(1000);
...
d_clob1         CLOB := EMPTY_CLOB() ;
d_dir_upload      VARCHAR(200);
v_PWD_DA_MODIFICARE varchar2(2);
v_NUMERO_TENTATIVI ad4_utenti.numero_tentativi%TYPE;
d_min_max_num_tentativi number;
BEGIN
  -- Blocco dei menu se password da cambiare
     -- rev.7 inizio
   select PWD_DA_MODIFICARE, NUMERO_TENTATIVI
   into v_PWD_DA_MODIFICARE, v_NUMERO_TENTATIVI
   from ad4_utenti
   where utente = p_utente;
   d_min_max_num_tentativi := ad4_caratteristica_accesso.GET_MIN_MAX_TENTATIVI_PWD ( p_utente);

if v_PWD_DA_MODIFICARE = 'SI' and nvl(registro_utility.leggi_stringa('PRODUCTS/AMV', 'Disabilita Menu se PWD da cambiare',0),'NO') = 'SI'
     and v_numero_tentativi >= nvl(d_min_max_num_tentativi,1 ) -- se non indicato blocco subito
     then
     -- rev.7 fine
         d_char :=  '<tr><td class="AFCErrorDataTD" width="100%"> MENU DISABILITATO! <br> Le credenziali di accesso sono scadute. </tr></td>';
         d_amount := LENGTH(d_char);
       dbms_lob.createTemporary(d_clob,TRUE,dbms_lob.SESSION);
...
-- ciclo per sezioni relative alla zona selezionata
   ...
   BEGIN
     ...
     IF c.immagine IS NOT NULL THEN
        d_immagine := '<img src="../'||d_dir_upload||'/images/'||c.immagine||'" border="0" alt="'||c.nome||'" align="AbsMiddle">&'||'nbsp;';
     ELSE
        d_immagine := '';
     END IF;
    ...
    -- espansione blocco
     IF c.zona_espansione = 'S' THEN
        IF c.zona_tipo = 'S' THEN  -- mostro solo sottosezioni
           d_content_row:= get_blocco_sottosezioni(c.id_sezione,0,p_zona);
        ELSIF c.zona_tipo = 'D' THEN  -- mostro solo documenti
           d_content_row:= get_blocco_documenti(p_utente, c.id_sezione, 0, p_zona, c.max_vis);
        ELSE  -- mostro sottosezioni e documenti
          d_content_row:= get_blocco_sottosezioni(c.id_sezione,0,p_zona);
          d_content_row:= d_content_row||get_blocco_documenti(p_utente, c.id_sezione, 0, p_zona, c.max_vis);
        END IF;
      d_row:= d_row||d_content_row;
     END IF;
...
BEGIN
   ...
   EXCEPTION WHEN NO_DATA_FOUND THEN
     ...
     BEGIN
      ...
      END;
   END;
   d_path := get_navigatore(d_padre, p_utente, p_modulo)||'&'||'nbsp;::&'||'nbsp;<a class="AFCDataLink" href="../common/AmvSezione.do?MVPD=0&'||'MVSZ='||d_id_sezione||'">'||d_nome||'</a>';
   RETURN d_path;
END GET_NAVIGATORE;
...
RETURN Afc_Html.TREE(
     ...
     P_WHERE =>  'decode(id_area,null,''C'', NVL(AMV_AREA.GET_DIRITTO_UTENTE('''||p_utente||''', ID_AREA),''Z'')) in (''U'',''A'',''V'',''C'')'
      );
...
-- documenti per la sezione selezionata
   ...
   LOOP
   BEGIN
     d_row := '<a href="AmvDocumentoInfo.do?ID='||c.id_documento||'">'||d_img||c.titolo||' ('||TO_CHAR(c.data_riferimento,'dd/mm/yyyy')||')'||'</a>&'||'nbsp;<br>';
     d_char:=d_char||d_row;
   END;
   END LOOP;
   IF d_nrows = 0 THEN
...
BEGIN
   ...
   LOOP
      BEGIN
         d_link:= '&'||'nbsp;|&'||'nbsp;'||'<a class="AFCHeaderBarMenu" href="../common/AmvSezione.do?MVPD=0&'||'MVSZ='||c.id_sezione||'">'||c.nome||'</a>';
...
/******************************************************************************
 ...
 ---- ---------- ------ ------------------------------------------------------
  ...
  2    20/05/2006 AO     Modificate condizioni su generazione blocco richieste da gestire
******************************************************************************/
d_row_start        VARCHAR2(1000);
d_row_int          VARCHAR2(1000);
d_row_end          VARCHAR2(1000);
...
-- creazione riga relativa alle richieste da gestire
   SELECT COUNT(1)
    ...
    FROM AMV_DOCUMENTI d, AMV_DOCUMENTI padri
         ...
         AND INSTR(padri.LINK, 'iter=N inoltro=I') < 1
       --AND d.autore != p_utente
       AND Amv_Area.get_diritto_utente(p_utente, d.id_area, d.id_tipologia) IN ('V','A','U')
   ;
   IF d_count > 0 THEN
...
BEGIN
   ...
   RETURN d_valore;
END;
FUNCTION IS_EMPTY(P_ID_SEZIONE NUMBER, P_UTENTE VARCHAR2) RETURN number IS
d_count     number(10) := 0;
d_is_empty  number(1) := 1;
BEGIN
   -- Lancio sulle sottosezioni
   for c in (select id_sezione
              from amv_sezioni
             start with id_sezione = p_id_sezione
           connect by prior id_sezione = id_padre)
   loop
      SELECT count(1)
        INTO d_count
        FROM AMV_DOCUMENTI
